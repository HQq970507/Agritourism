...
const querySearchAsync = async (queryString, cb) => {
  ...
  try {
    ...
    // 兼容后端返回 { categoryList: [...] } 的搜索接口（如分类搜索），转为字符串数组并前端筛选
    if (data && !Array.isArray(data) && Array.isArray(data.categoryList)) {
      const keyword = (queryString || '').toLowerCase()
      data = data.categoryList
        .filter((item) => (item.name || '').toLowerCase().includes(keyword))
        .map((item) => item.name)
    }
    SearchResult.value = (Array.isArray(data) ? data : []).map((item) => ({
      value: item
    }))
    // 调用回调函数，传入处理后的搜索结果
